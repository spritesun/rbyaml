...
class RbYAML_Unit_Tests < Test::Unit::TestCase
        #
        # Convert between RbYAML and the object to verify correct parsing and
        # emitting
        #
        def assert_to_yaml( obj, yaml )
                assert_equal( obj, RbYAML::load( yaml ) )
                assert_equal( obj, RbYAML::parse( yaml ).transform )
        assert_equal( obj, RbYAML::load( obj.to_yaml ) )
                assert_equal( obj, RbYAML::parse( obj.to_yaml ).transform )
        assert_equal( obj, RbYAML::load(
    ...
    def assert_path_segments( path, segments )
        RbYAML::YPath.each_path( path ) { |choice|
        ...
        #
        # Tests modified from 00basic.t in RbYAML.pm
        #
        def test_basic_map
                # Simple map
                assert_parse_only(
        ...
        def test_basic_strings
                ...
                assert_parse_only(
                        { 1 => 'simple string', 2 => 42, 3 => '1 Single Quoted String',
                          4 => 'RbYAML\'s Double "Quoted" String', 5 => "A block\n  with several\n    lines.\n",
                          6 => "A \"chomped\" block", 7 => "A folded\n string\n", 8 => ": started string" },
                          <<EOY
1: simple string
2: 42
3: '1 Single Quoted String'
4: "RbYAML's Double \\\"Quoted\\\" String"
5: |
  A block
    with several
      lines.
...
EOY
        ...
        def test_spec_root_fold
                y = RbYAML::load( <<EOY
--- >
This RbYAML stream contains a single text value.
The next stream is a log file - a sequence of
log entries. Adding an entry to the log is a
simple matter of appending it at the end.
EOY
                )
                assert_equal( y, "This RbYAML stream contains a single text value. The next stream is a log file - a sequence of log entries. Adding an entry to the log is a simple matter of appending it at the end.\n" )
        end
...
EOY
        ...
        RbYAML.add_domain_type( "domain.tld,2002", 'invoice', &customer_proc )
        RbYAML.add_domain_type( "domain.tld,2002", 'customer', &customer_proc )
...
EOY
        ...
        def test_spec_url_escaping
                RbYAML.add_domain_type( "domain.tld,2002", "type0" ) { |type, val|
                        "ONE: #{val}"
                }
                RbYAML.add_domain_type( "domain.tld,2002", "type%30" ) { |type, val|
                        "TWO: #{val}"
                }
                assert_parse_only(
...
EOY
        ...
        RbYAML.add_domain_type( "somewhere.com,2002", 'type' ) { |type, val|
            "SOMEWHERE: #{val}"
        }
                assert_parse_only(
...
EOY
        ...
        def test_spec_application_family
                ...
                RbYAML.add_domain_type( "clarkevans.com,2002", 'graph/shape' ) { |type, val|
                        if Array === val
                                val << "Shape Container"
                                val
                        else
                ...
                RbYAML.add_domain_type( "clarkevans.com,2002", 'graph/circle', &one_shape_proc )
                RbYAML.add_domain_type( "clarkevans.com,2002", 'graph/line', &one_shape_proc )
                RbYAML.add_domain_type( "clarkevans.com,2002", 'graph/text', &one_shape_proc )
                assert_parse_only(
...
EOY
        ...
        assert_to_yaml( RbYAML_Tests::StructTest.new( 123 ), <<EOY )
--- !ruby/struct:RbYAML_Tests::StructTest
c: 123
EOY
...
EOY
        ...
        #
        # Test the RbYAML::Stream class -- INACTIVE at the moment
        #
        def test_document
                y = RbYAML::Stream.new( :Indent => 2, :UseVersion => 0 )
                y.add(
                        { 'hi' => 'hello', 'map' =>
                                { 'good' => 'two' },
                          'time' => Time.now,
...
EOY
    ...
    def test_circular_references
        a = []; a[0] = a; a[1] = a
        inspect_str = "[[...], [...]]"
        assert_equal( inspect_str, RbYAML::load( a.to_yaml ).inspect )
    end
...
if $0 == __FILE__
  suite = Test::Unit::TestSuite.new('RbYAML')
